# QrCodeGenerator Ruby SDK



The Ruby SDK for the QrCodeGenerator API — an entity-oriented client using idiomatic Ruby conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `client.Qrn` — with named operations (`load`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
```ruby
client = QrCodeGeneratorSDK.test(testopts, sdkopts)
```
...
raise a `QrCodeGeneratorError` (a `StandardError` subclass), so wrap
...
```
rb/
├── QrCodeGenerator_sdk.rb       -- Main SDK module
...
The main module (`QrCodeGenerator_sdk`) exports the SDK class
