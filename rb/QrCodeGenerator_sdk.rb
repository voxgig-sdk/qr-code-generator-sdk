...
require_relative 'QrCodeGenerator_types'
...
class QrCodeGeneratorSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = QrCodeGeneratorHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = QrCodeGeneratorHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, QrCodeGeneratorFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    QrCodeGeneratorUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = QrCodeGeneratorHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = QrCodeGeneratorHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = QrCodeGeneratorHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    rescue QrCodeGeneratorError => err
    ...
    if fetched.is_a?(Hash)
      status = QrCodeGeneratorHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = QrCodeGeneratorError.new(
        "graphql_error", "QrCodeGeneratorSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = QrCodeGeneratorSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
