...
class QrnEntity
  ...
  def make
    opts = @_entopts.dup
    QrnEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = QrCodeGeneratorHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = QrCodeGeneratorHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  #   as an empty match, so client.Qrn.load works with no args).
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = QrCodeGeneratorHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
