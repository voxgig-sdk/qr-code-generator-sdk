...
module QrCodeGeneratorUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = QrCodeGeneratorResponse.new({})
    ...
    if fetch_err
      ...
      response = QrCodeGeneratorResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = QrCodeGeneratorResponse.new(fetched)
    else
