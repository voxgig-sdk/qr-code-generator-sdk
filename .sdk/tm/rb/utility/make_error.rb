...
module QrCodeGeneratorUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = QrCodeGeneratorContext.new({}, nil)
    end
    op = ctx.op || QrCodeGeneratorOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || QrCodeGeneratorResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(QrCodeGeneratorError) ? err.msg : err.to_s
    ...
    sdk_err = QrCodeGeneratorError.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(QrCodeGeneratorError)
