...
class QrCodeGeneratorMakeRequest
{
    public static function call(QrCodeGeneratorContext $ctx): array
    {
        ...
        if ($fetch_err) {
            ...
            $response = new QrCodeGeneratorResponse(['err' => $ctx->make_error('request_no_response', 'response: undefined')]);
        } elseif (is_array($fetched)) {
            $response = new QrCodeGeneratorResponse($fetched);
        } else {
