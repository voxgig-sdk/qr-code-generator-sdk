...
// QrCodeGenerator SDK feature test
//
...
class FtCtrl extends QrCodeGeneratorControl
{
...
class FtClient
{
    ...
    private QrCodeGeneratorUtility $utility;

    public function __construct(QrCodeGeneratorUtility $utility, array $options)
    {
    ...
    public function get_utility(): QrCodeGeneratorUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (QrCodeGeneratorContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public QrCodeGeneratorContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (QrCodeGeneratorContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = QrCodeGeneratorConfig::make_feature($name);
    ...
    private static function buildUrl(QrCodeGeneratorSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof QrCodeGeneratorError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new QrCodeGeneratorResponse($fetched);
        }
    ...
    private function fail(QrCodeGeneratorContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (QrCodeGeneratorContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
