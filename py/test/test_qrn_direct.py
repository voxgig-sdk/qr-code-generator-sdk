...
class TestQrnDirect:
    ...
    def test_should_direct_load_qrn(self):
        setup = _qrn_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-qrn", "live" if setup["live"] else "unit")
        if _skip:
...
def _qrn_direct_setup(mockres):
    ...
    if live:
        ...
        client = QrCodeGeneratorSDK(merged_opts)
        return {
            "client": client,
            "calls": calls,
            "live": True,
