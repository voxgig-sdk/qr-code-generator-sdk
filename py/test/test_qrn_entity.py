...
class TestQrnEntity:
    ...
    def test_should_create_instance(self):
        ...
        ent = testsdk.Qrn(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _qrn_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "qrn." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set QR_CODE_GENERATOR_TEST_QRN_ENTID JSON to run live")
        client = setup["client"]
...
def _qrn_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/qrn/QrnTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("QR_CODE_GENERATOR_TEST_QRN_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)

    if env.get("QR_CODE_GENERATOR_TEST_LIVE") == "TRUE":
        merged_opts = vs.merge([
            {
            },
            extra or {},
        ])
        client = QrCodeGeneratorSDK(helpers.to_map(merged_opts))

    _live = env.get("QR_CODE_GENERATOR_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("QR_CODE_GENERATOR_TEST_EXPLAIN") == "TRUE",
        "live": _live,
