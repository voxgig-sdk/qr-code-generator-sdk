# QrCodeGenerator SDK test runner
...
class QrCodeGeneratorTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            QrCodeGeneratorTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = QrCodeGeneratorTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = QrCodeGeneratorTestRunner.getenv("QR_CODE_GENERATOR_TEST_LIVE")
        override = QrCodeGeneratorTestRunner.getenv("QR_CODE_GENERATOR_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = QrCodeGeneratorTestRunner.getenv(key)
        ...
        explain = QrCodeGeneratorTestRunner.getenv("QR_CODE_GENERATOR_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if QrCodeGeneratorTestRunner._test_control is not None:
            return QrCodeGeneratorTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                QrCodeGeneratorTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            QrCodeGeneratorTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = QrCodeGeneratorTestRunner.load_test_control()
...
def load_env_local():
    QrCodeGeneratorTestRunner.load_env_local()


def env_override(m):
    return QrCodeGeneratorTestRunner.env_override(m)


def entity_data(v):
    return QrCodeGeneratorTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return QrCodeGeneratorTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return QrCodeGeneratorTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return QrCodeGeneratorTestRunner.load_test_control()


def live_client_options():
    return QrCodeGeneratorTestRunner.live_client_options()


def live_delay_ms():
    return QrCodeGeneratorTestRunner.live_delay_ms()
