...
from qrcodegenerator_sdk.core.control import QrCodeGeneratorControl
...
from qrcodegenerator_sdk.core.helpers import get_ctx_prop, to_map
...
class QrCodeGeneratorContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        # Ctrl
        self.ctrl = QrCodeGeneratorControl()
        ...
        if isinstance(sp, QrCodeGeneratorSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, QrCodeGeneratorResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, QrCodeGeneratorResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def resolve_op(self, opname):
        ...
        if opname == "":
            return QrCodeGeneratorOperation({})
    ...
    def make_error(self, code, msg):
        return QrCodeGeneratorError(code, msg, self)
