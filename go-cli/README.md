...
boru-driven command-line client **and** interactive REPL for the QrCodeGenerator
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/qr-code-generator-cli)
make build

# 2. See usage (words, entities, env vars)
./qr-code-generator-cli --help

# 3. Provide credentials once, via the environment
export QR_CODE_GENERATOR_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./qr-code-generator-cli load 1 qrn            # {id:1} shorthand
./qr-code-generator-cli load '{id:1}' qrn       # explicit match map

# 5. Override the API base URL for a single call
QR_CODE_GENERATOR_BASE=https://api.example.com ./qr-code-generator-cli load 1 qrn

# 6. No arguments -> interactive REPL
./qr-code-generator-cli
qr-code-generator> load 1 qrn
qr-code-generator> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/qr-code-generator-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/qr-code-generator-cli load 1 qrn
   ```
...
```sh
./qr-code-generator-cli load 1 qrn          # scalar shorthand for {id:1}
./qr-code-generator-cli load '{id:1}' qrn     # explicit match map
```
...
./qr-code-generator-cli load 1 qrn
```
...
Run with no arguments to open a REPL (prompt `qr-code-generator>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `qrn`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/qr-code-generator-cli`. |
...
(`./qr-code-generator-cli <expr>`) and interactively (the REPL), and expressions compose the
