// Command QrCodeGenerator-cli is a CLI + REPL for the QrCodeGenerator SDK,
...
const prompt = "qr-code-generator"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "qrn"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      QR_CODE_GENERATOR_APIKEY (api key), QR_CODE_GENERATOR_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: QR_CODE_GENERATOR_APIKEY carries the API key and
	// QR_CODE_GENERATOR_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("QR_CODE_GENERATOR_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("QR_CODE_GENERATOR_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewQrCodeGeneratorSDK(opts)
