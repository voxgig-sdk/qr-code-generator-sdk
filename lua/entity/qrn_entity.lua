...
local QrnEntity = {}
QrnEntity.__index = QrnEntity


function QrnEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, QrnEntity)
  self._name = "qrn"
  self._client = client
...
function QrnEntity:get_name()
  return self._name
end


function QrnEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return QrnEntity.new(self._client, opts)
end
...
function QrnEntity:mark_deleted()
  self._deleted = true
end


function QrnEntity:deleted()
  return true == self._deleted
end


function QrnEntity:data_set(args)
  if args ~= nil then
...
function QrnEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function QrnEntity:match_set(args)
  if args ~= nil then
...
function QrnEntity:stream(action, args, callopts)
...
function QrnEntity:_run_op(ctx, post_done)
  local utility = self._utility
