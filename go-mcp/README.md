...
[MCP](https://modelcontextprotocol.io) server exposing the QrCodeGenerator SDK as
two agent tools — `qr-code-generator_list` and `qr-code-generator_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/qr-code-generator-mcp)
make build

# 2. Provide credentials via the environment
export QR_CODE_GENERATOR_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user qr-code-generator \
  -- /absolute/path/to/qr-code-generator-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./qr-code-generator-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// qr-code-generator_list: first page of records
{ "entity": "qrn" }
{ "entity": "qrn", "query": { } }

// qr-code-generator_load: one record by id
{ "entity": "qrn", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/qr-code-generator-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user qr-code-generator \
     -- "$PWD"/dist/*/qr-code-generator-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `qr-code-generator_list` and `qr-code-generator_load` tools now appear
   in new sessions. Ask the agent to *"list qrn using qr-code-generator"*
   and it calls `qr-code-generator_list` with `{"entity":"qrn"}`.
...
```sh
export QR_CODE_GENERATOR_APIKEY=sk_live_xxx            # API key
export QR_CODE_GENERATOR_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `qr-code-generator_list` tool
...
### Call the `qr-code-generator_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `qr-code-generator_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `qr-code-generator_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `QR_CODE_GENERATOR_APIKEY` | API key sent with every request. |
| `QR_CODE_GENERATOR_BASE` | Optional override of the API base URL. |
...
```sh
./qr-code-generator-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"qr-code-generator_load","arguments":{"entity":"qrn","query":{"id":1}}}}'
```
