...
(0, node_test_1.describe)('QrnEntity', async () => {
    // Per-test live pacing. Delay is read from sdk-test-control.json's
    // `test.live.delayMs`; only sleeps when QR_CODE_GENERATOR_TEST_LIVE=TRUE.
    (0, node_test_1.afterEach)((0, utility_1.liveDelay)('QR_CODE_GENERATOR_TEST_LIVE'));
    (0, node_test_1.test)('instance', async () => {
        const testsdk = __1.QrCodeGeneratorSDK.test();
    ...
    (0, node_test_1.test)('basic', async (t) => {
        ...
        for (const op of ['load']) {
            if (!live && (0, utility_1.maybeSkipControl)(t, 'entityOp', 'qrn.' + op, live))
                return;
        }
        ...
        let qrn_ref01_data = Object.values(setup.data.existing.qrn)[0];
        // LOAD
        const qrn_ref01_ent = client.Qrn();
        const qrn_ref01_match_dt0 = {};
        const qrn_ref01_data_dt0 = (await qrn_ref01_ent.load(qrn_ref01_match_dt0)).data();
        (0, node_assert_1.default)(null != qrn_ref01_data_dt0);
    });
});
function basicSetup(extra) {
    ...
    const entityDataFile = node_path_1.default.resolve(__dirname, '../../../../.sdk/test/entity/qrn/QrnTestData.json');
    // TODO: file ready util needed?
    const entityDataSource = Fs.readFileSync(entityDataFile).toString('utf8');
    // TODO: need a xlang JSON parse utility in voxgig/struct with better error msgs
    const entityData = JSON.parse(entityDataSource);
    options.entity = entityData.existing;
    let client = __1.QrCodeGeneratorSDK.test(options, extra);
    const struct = client.utility().struct;
    const merge = struct.merge;
    const transform = struct.transform;
    let idmap = transform(['qrn01', 'qrn02', 'qrn03'], {
        '`$PACK`': ['', {
                '`$KEY`': '`$COPY`',
    ...
    if (live) {
        ...
        client = new __1.QrCodeGeneratorSDK(merge([
...
//# sourceMappingURL=QrnEntity.test.js.map