...
`client.Qrn()` — each with a small set of operations (`load`)
...
| `tester(testopts?, sdkopts?)` | `QrCodeGeneratorSDK` | Create a test-mode client instance. |

#### Static methods

| Method | Returns | Description |
| --- | --- | --- |
| `QrCodeGeneratorSDK.test(testopts?, sdkopts?)` | `QrCodeGeneratorSDK` | Create a test-mode client. |
...
| `client` | `client(): QrCodeGeneratorSDK` | Return the parent SDK client. |
...
qr-code-generator/
├── src/
│   ├── QrCodeGeneratorSDK.ts        # Main SDK class
