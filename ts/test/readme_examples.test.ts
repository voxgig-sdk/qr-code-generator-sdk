...
//     (\`new QrCodeGeneratorSDK(...)\` / \`QrCodeGeneratorSDK.test(...)\` become
//     \`QrCodeGeneratorSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'QrCodeGeneratorSDK'
...
const TEST_SEED = {"entity":{"qrn":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new QrCodeGeneratorSDK(...)\` and any
// \`QrCodeGeneratorSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+QrCodeGeneratorSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/QrCodeGeneratorSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
