...
import (
	...
	omni "github.com/voxgig-sdk/qr-code-generator-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.QrCodeGeneratorError); is {
				out := map[string]any{
					"name":    "QrCodeGeneratorError",
...
func omniCtx(arg any, client *sdk.QrCodeGeneratorSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.QrCodeGeneratorSDK); is {
			client = live
