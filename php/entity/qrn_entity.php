...
// QrCodeGenerator SDK Qrn entity
...
class QrnEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new QrnEntity($this->_client, $opts);
    }

    /**
     * @param Qrn|array $args Qrn data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = QrCodeGeneratorHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Qrn|array The current Qrn data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Qrn fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = QrCodeGeneratorHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Qrn fields).
     */
     ...
     * Load a single Qrn.
     *
     * @param QrnLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed QrnLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Qrn|array The loaded Qrn as an assoc-array at the
     *   SDK boundary; throws QrCodeGeneratorError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = QrCodeGeneratorHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
