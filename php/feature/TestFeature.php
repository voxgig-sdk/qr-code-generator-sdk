...
// QrCodeGenerator SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class QrCodeGeneratorTestFeature extends QrCodeGeneratorBaseFeature
{
    ...
    public function init(QrCodeGeneratorContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (QrCodeGeneratorContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = QrCodeGeneratorParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (QrCodeGeneratorContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(QrCodeGeneratorContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = QrCodeGeneratorParam::call($ctx, $k);
