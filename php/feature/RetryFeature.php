...
// QrCodeGenerator SDK retry feature
...
class QrCodeGeneratorRetryFeature extends QrCodeGeneratorBaseFeature
{
    ...
    public function init(QrCodeGeneratorContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (QrCodeGeneratorContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(QrCodeGeneratorContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(QrCodeGeneratorContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
