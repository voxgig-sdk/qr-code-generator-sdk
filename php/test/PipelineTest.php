...
// QrCodeGenerator SDK pipeline test
//
...
{
    private static function utility(): QrCodeGeneratorUtility
    {
        return new QrCodeGeneratorUtility();
    }
    ...
    private static function ctx(array $over = []): QrCodeGeneratorContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new QrCodeGeneratorContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new QrCodeGeneratorOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof QrCodeGeneratorError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new QrCodeGeneratorOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = QrCodeGeneratorMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new QrCodeGeneratorOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = QrCodeGeneratorMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new QrCodeGeneratorOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = QrCodeGeneratorMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new QrCodeGeneratorSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = QrCodeGeneratorMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            ...
            'result' => new QrCodeGeneratorResult([]),
        ]);
        [, $err] = QrCodeGeneratorMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new QrCodeGeneratorSpec([]),
            'response' => null,
            'result' => new QrCodeGeneratorResult([]),
        ]);
        [, $err] = QrCodeGeneratorMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new QrCodeGeneratorSpec(['step' => 's']),
            'response' => new QrCodeGeneratorResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new QrCodeGeneratorResult([]),
        ]);
        [, $err] = QrCodeGeneratorMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new QrCodeGeneratorSpec(['step' => 's']),
            'response' => new QrCodeGeneratorResponse(self::resp(200, ['v' => 1])),
            'result' => new QrCodeGeneratorResult([]),
        ]);
        [, $err] = QrCodeGeneratorMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new QrCodeGeneratorSpec(['step' => 's']),
            'response' => new QrCodeGeneratorResponse(self::resp(200, ['v' => 2])),
            'result' => new QrCodeGeneratorResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        QrCodeGeneratorMakeResponse::call($ctx);
    ...
    {
        $preset = new QrCodeGeneratorResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new QrCodeGeneratorSpec([]),
            'response' => new QrCodeGeneratorResponse([]),
            'result' => new QrCodeGeneratorResult([]),
        ...
        [$got, $err] = QrCodeGeneratorMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new QrCodeGeneratorResult([])]);
        [, $err] = QrCodeGeneratorMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new QrCodeGeneratorSpec([]), 'result' => null]);
        [, $err] = QrCodeGeneratorMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new QrCodeGeneratorSpec(['step' => 's']),
            'result' => new QrCodeGeneratorResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new QrCodeGeneratorOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = QrCodeGeneratorMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new QrCodeGeneratorSpec(['step' => 's']),
            'result' => new QrCodeGeneratorResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new QrCodeGeneratorOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = QrCodeGeneratorMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new QrCodeGeneratorResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new QrCodeGeneratorSpec([]),
            'result' => new QrCodeGeneratorResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = QrCodeGeneratorMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = QrCodeGeneratorMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new QrCodeGeneratorError('boom', 'boom');
        $utility->fetcher = function (QrCodeGeneratorContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new QrCodeGeneratorSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = QrCodeGeneratorMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (QrCodeGeneratorContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = QrCodeGeneratorMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(QrCodeGeneratorResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        QrCodeGeneratorMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (QrCodeGeneratorContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new QrCodeGeneratorSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = QrCodeGeneratorMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new QrCodeGeneratorResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new QrCodeGeneratorSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = QrCodeGeneratorMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = QrCodeGeneratorMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = QrCodeGeneratorMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new QrCodeGeneratorResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, QrCodeGeneratorDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new QrCodeGeneratorResult(['ok' => false])]);
        $this->expectException(QrCodeGeneratorError::class);
        QrCodeGeneratorDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new QrCodeGeneratorResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', QrCodeGeneratorMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        QrCodeGeneratorMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new QrCodeGeneratorBaseFeature();
        $b = new QrCodeGeneratorBaseFeature();
        QrCodeGeneratorFeatureAdd::call($ctx, $a);
        QrCodeGeneratorFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): QrCodeGeneratorBaseFeature
    {
        $f = new QrCodeGeneratorBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        QrCodeGeneratorFeatureAdd::call($ctx, self::named_feature('a'));
        QrCodeGeneratorFeatureAdd::call($ctx, self::named_feature('b'));
        $this->assertSame(['a', 'b'], $names());

        $before = self::named_feature('z1');
        $before->_options = ['__before__' => 'b'];
        QrCodeGeneratorFeatureAdd::call($ctx, $before);
        $this->assertSame(['a', 'z1', 'b'], $names());

        $after = self::named_feature('z2');
        $after->_options = ['__after__' => 'a'];
        QrCodeGeneratorFeatureAdd::call($ctx, $after);
        $this->assertSame(['a', 'z2', 'z1', 'b'], $names());

        $replace = self::named_feature('z3');
        $replace->_options = ['__replace__' => 'z1'];
        QrCodeGeneratorFeatureAdd::call($ctx, $replace);
        ...
        QrCodeGeneratorFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): QrCodeGeneratorContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new QrCodeGeneratorSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = QrCodeGeneratorPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        QrCodeGeneratorPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new QrCodeGeneratorResponse(['headers' => null]),
            'result' => new QrCodeGeneratorResult([]),
        ]);
        QrCodeGeneratorResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        QrCodeGeneratorResultBody::call($ctx);
