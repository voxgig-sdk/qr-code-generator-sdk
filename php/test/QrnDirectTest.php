...
class QrnDirectTest extends TestCase
{
    public function test_direct_load_qrn(): void
    {
        $setup = qrn_direct_setup(["id" => "direct01"]);
        [$_shouldSkip, $_reason] = Runner::is_control_skipped("direct", "direct-load-qrn", $setup["live"] ? "live" : "unit");
        if ($_shouldSkip) {
...
function qrn_direct_setup($mockres)
{
    ...
    if ($live) {
        ...
        $client = new QrCodeGeneratorSDK($merged_opts);
        return [
            "client" => $client,
            "calls" => $calls,
            "live" => true,
